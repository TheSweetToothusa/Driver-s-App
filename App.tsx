...
const AdminPanel: React.FC<{ role: AppRole; deliveries: Delivery[]; allUsers: UserAccount[]; setAllUsers: React.Dispatch<React.SetStateAction<UserAccount[]>>; }> = ({ role, deliveries, allUsers, setAllUsers }) => {
  const [activeSection, setActiveSection] = useState<'FEES' | 'DRIVERS' | 'RESCHEDULE' | 'MESSAGES' | null>(null);
  const [addDriverExpanded, setAddDriverExpanded] = useState(false);
  const [defaultDriverExpanded, setDefaultDriverExpanded] = useState(false);
  const [newDriver, setNewDriver] = useState({ name: '', pin: '', phone: '', vehicle: '' });
  const [addError, setAddError] = useState('');
  const [addSuccess, setAddSuccess] = useState('');
  const [resetPinId, setResetPinId] = useState<string | null>(null);
  ...
  const handleSaveDefaultDriver = async () => {
    ...
    setDefaultDriverExpanded(false);
  };
  ...
  // Compute delivery fees for the date range
  ...
  return (
    <div className="flex flex-col h-full bg-stone-50">
      {/* TOP ADMIN BAR — The "Big Four" Action Tiles */}
      <div className="sticky top-0 bg-stone-50 z-20 px-4 pt-4 pb-3">
        <div className="grid grid-cols-4 gap-2">
          {/* FEES — Yellow highlight */}
          <button onClick={() => setActiveSection(activeSection === 'FEES' ? null : 'FEES')}
            className={`flex flex-col items-center justify-center py-3 rounded-2xl transition-all ${activeSection === 'FEES' ? 'bg-amber-400 shadow-lg scale-105' : 'bg-amber-100 border border-amber-200'}`}>
            <DollarSign size={20} className={activeSection === 'FEES' ? 'text-amber-900' : 'text-amber-600'} />
            <span className={`text-[9px] font-black uppercase mt-1 ${activeSection === 'FEES' ? 'text-amber-900' : 'text-amber-700'}`}>Fees</span>
          </button>

          {/* DRIVERS */}
          <button onClick={() => setActiveSection(activeSection === 'DRIVERS' ? null : 'DRIVERS')}
            className={`flex flex-col items-center justify-center py-3 rounded-2xl transition-all ${activeSection === 'DRIVERS' ? 'bg-black text-white shadow-lg scale-105' : 'bg-white border border-stone-200 text-stone-600'}`}>
            <Users size={20} />
            <span className="text-[9px] font-black uppercase mt-1">Drivers</span>
          </button>

          {/* RESCHEDULE */}
          <button onClick={() => setActiveSection(activeSection === 'RESCHEDULE' ? null : 'RESCHEDULE')}
            className={`flex flex-col items-center justify-center py-3 rounded-2xl transition-all ${activeSection === 'RESCHEDULE' ? 'bg-black text-white shadow-lg scale-105' : 'bg-white border border-stone-200 text-stone-600'}`}>
            <Calendar size={20} />
          ...
          <button onClick={() => setActiveSection(activeSection === 'MESSAGES' ? null : 'MESSAGES')}
            className={`flex flex-col items-center justify-center py-3 rounded-2xl transition-all ${activeSection === 'MESSAGES' ? 'bg-black text-white shadow-lg scale-105' : 'bg-white border border-stone-200 text-stone-600'}`}>
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-28">
        ...
        {activeSection === 'FEES' && (
          <div className="space-y-3 animate-in fade-in slide-in-from-top-2 duration-200">
            ...
            <div className="p-4 bg-white rounded-2xl border border-stone-100 shadow-sm">
              <p className="text-[10px] font-black uppercase text-stone-400 mb-2">Quick ZIP Lookup</p>
              <div className="flex gap-2">
                <input type="text" placeholder="Enter ZIP" maxLength={5} inputMode="numeric"
                  value={feeZip} onChange={e => { setFeeZip(e.target.value.replace(/\D/g, '').slice(0,5)); setFeeResult(null); }}
                  className="flex-1 bg-stone-50 border border-stone-200 rounded-xl px-4 py-3 text-lg font-black text-center tracking-widest outline-none focus:border-amber-400" />
                <button onClick={handleZipLookup} disabled={feeZip.length !== 5}
                  className={`px-6 rounded-xl font-black uppercase text-sm transition-all ${feeZip.length === 5 ? 'bg-amber-400 text-amber-900 active:scale-95' : 'bg-stone-100 text-stone-300'}`}>
                  Check
                </button>
              </div>
              {feeResult !== null && (
                <div className={`mt-3 p-3 rounded-xl text-center font-black ${feeResult === -1 ? 'bg-red-50 text-red-600' : 'bg-green-50 text-green-700'}`}>
                  {feeResult === -1 ? 'ZIP not found' : `$${feeResult.toFixed(2)}`}
                </div>
              )}
            </div>

            {/* Date range fee calculator */}
            <div className="p-4 bg-white rounded-2xl border border-stone-100 shadow-sm space-y-3">
              <p className="text-[10px] font-black uppercase text-stone-400">Delivery Fee Report</p>
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="text-[8px] font-black uppercase text-stone-300 mb-1 block">From</label>
                  <input type="date" value={feeStart} onChange={e => { setFeeStart(e.target.value); setFeeCalculated(false); }}
                    className="w-full bg-stone-50 border border-stone-100 rounded-xl px-3 py-2 text-xs font-bold outline-none" />
                </div>
                <div>
                  <label className="text-[8px] font-black uppercase text-stone-300 mb-1 block">To</label>
              ...
              <select value={feeDriverFilter} onChange={e => { setFeeDriverFilter(e.target.value); setFeeCalculated(false); }}
                className="w-full bg-stone-50 border border-stone-100 rounded-xl px-3 py-2.5 text-sm font-bold outline-none appearance-none">
              ...
              <button onClick={() => { setCalcStart(feeStart); setCalcEnd(feeEnd); setFeeCalculated(true); }}
                className="w-full py-3 bg-amber-400 text-amber-900 rounded-xl font-black uppercase text-sm active:scale-95 transition-all">
                Calculate
              </button>
              {feeCalculated && (
                <div className="p-4 bg-black rounded-xl flex items-center justify-between">
                  <div>
                    <p className="text-[9px] font-black uppercase text-white/50">{feeDriverFilter === 'ALL' ? 'All Drivers' : allUsers.find(u => u.id === feeDriverFilter)?.name}</p>
                    <p className="text-[10px] text-white/60">{grandCount} deliveries</p>
                  </div>
        ...
        {activeSection === 'DRIVERS' && (
          <div className="space-y-3 animate-in fade-in slide-in-from-top-2 duration-200">
            ...
            {/* Default Driver — Accordion */}
            <div className="bg-white rounded-2xl border border-stone-100 shadow-sm overflow-hidden">
              <button onClick={() => setDefaultDriverExpanded(!defaultDriverExpanded)}
                className="w-full flex items-center justify-between p-4">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-full bg-green-500 flex items-center justify-center text-white font-black text-sm">
                    {currentDefaultDriver?.name?.charAt(0) || '?'}
                  </div>
                  <div className="text-left">
                    <p className="text-[9px] font-black uppercase text-stone-400">Default Driver</p>
                    <p className="text-sm font-black text-stone-800">{currentDefaultDriver?.name || 'Katie'}</p>
                  </div>
                </div>
                <ChevronDown size={20} className={`text-stone-400 transition-transform ${defaultDriverExpanded ? 'rotate-180' : ''}`} />
              </button>
              {defaultDriverExpanded && (
                <div className="px-4 pb-4 pt-0 border-t border-stone-100 space-y-3">
                  <select value={defaultDriverId} onChange={e => setDefaultDriverId(e.target.value)}
                    className="w-full bg-stone-50 border border-stone-100 rounded-xl px-4 py-3 text-sm font-bold outline-none appearance-none">
                    <option value="">— Select —</option>
                    {allUsers.filter(u => (u.role === 'DRIVER' || u.role === 'MANAGER') && u.isActive).map(u => (
                      <option key={u.id} value={u.id}>{u.name}</option>
                    ))}
                  </select>
                  {defaultDriverSaved && <p className="text-xs font-black text-green-600 text-center">Saved</p>}
                  <button onClick={handleSaveDefaultDriver} disabled={!defaultDriverId}
                    className={`w-full py-3 rounded-xl font-black uppercase text-sm ${defaultDriverId ? 'bg-black text-white active:scale-95' : 'bg-stone-200 text-stone-400'}`}>
                    Save
                  </button>
                </div>
              )}
            ...
            <div className="bg-white rounded-2xl border border-stone-100 shadow-sm overflow-hidden">
              <button onClick={() => setAddDriverExpanded(!addDriverExpanded)}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <p className="text-sm font-black text-stone-800">Add New Driver</p>
                </div>
              ...
              {addDriverExpanded && (
                <div className="px-4 pb-4 pt-0 border-t border-stone-100 space-y-2">
                  <input type="text" placeholder="Name" value={newDriver.name} onChange={e => setNewDriver(p => ({ ...p, name: e.target.value }))}
                    className="w-full bg-stone-50 border border-stone-100 rounded-xl px-4 py-2.5 text-sm font-medium outline-none" />
                  <input type="text" placeholder="4-digit PIN" maxLength={4} inputMode="numeric" value={newDriver.pin}
                    ...
                    className="w-full bg-stone-50 border border-stone-100 rounded-xl px-4 py-2.5 text-sm font-medium outline-none" />
                  <input type="tel" placeholder="Phone" value={newDriver.phone} onChange={e => setNewDriver(p => ({ ...p, phone: e.target.value }))}
                    className="w-full bg-stone-50 border border-stone-100 rounded-xl px-4 py-2.5 text-sm font-medium outline-none" />
                  <input type="text" placeholder="Vehicle" value={newDriver.vehicle} onChange={e => setNewDriver(p => ({ ...p, vehicle: e.target.value }))}
                    className="w-full bg-stone-50 border border-stone-100 rounded-xl px-4 py-2.5 text-sm font-medium outline-none" />
            ...
            {/* Driver List — Clean cards */}
            <div className="space-y-2">
              <p className="text-[9px] font-black uppercase text-stone-400 px-1">Active Drivers</p>
              {drivers.length === 0 && (
                <div className="text-center py-8 text-stone-300">
                  <Users size={24} className="mx-auto mb-2" />
                  <p className="text-xs font-black uppercase">No drivers yet</p>
                </div>
              )}
              {drivers.map(u => (
                <div key={u.id} className={`bg-white rounded-2xl border shadow-sm overflow-hidden ${!u.isActive ? 'opacity-50 border-stone-200' : 'border-stone-100'}`}>
                  <div className="p-4 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className={`w-2 h-2 rounded-full ${u.isActive ? 'bg-green-500' : 'bg-stone-300'}`} />
                      <div>
                        <p className="font-black text-stone-900">{u.name}</p>
                        <p className="text-[11px] text-stone-400">{u.phone || 'No phone'}{u.vehicle ? ` · ${u.vehicle}` : ''}</p>
                      </div>
                    </div>
                    ...
                    <div className="px-4 pb-4 pt-0 border-t border-stone-100 space-y-2">
                      <div className="flex gap-2 pt-3">
                        <button onClick={() => toggleActive(u)}
                          className={`flex-1 py-2.5 rounded-xl font-black uppercase text-xs ${u.isActive ? 'bg-red-50 text-red-500' : 'bg-green-50 text-green-600'}`}>
                          {u.isActive ? 'Deactivate' : 'Activate'}
                        </button>
                      </div>
                      <div className="flex gap-2">
                        <input type="text" placeholder="New 4-digit PIN" maxLength={4} inputMode="numeric"
                          value={newPinVal} onChange={e => setNewPinVal(e.target.value.replace(/\D/g, '').slice(0, 4))}
                          className="flex-1 bg-stone-50 border border-stone-100 rounded-xl px-4 py-2.5 text-sm font-black text-center tracking-widest outline-none" />
                        <button onClick={() => handleResetPin(u.id)} disabled={newPinVal.length !== 4}
                          className={`px-4 rounded-xl font-black text-xs uppercase ${newPinVal.length === 4 ? 'bg-black text-white' : 'bg-stone-100 text-stone-300'}`}>
                          Reset PIN
                        </button>
                      </div>
                    </div>
        ...
        {/* DEFAULT STATE — Quick summary when no section is open */}
        {activeSection === null && (
          <div className="space-y-4 pt-2 animate-in fade-in duration-200">
            {/* Quick stats */}
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-white rounded-2xl border border-stone-100 p-4 text-center">
                <p className="text-3xl font-black text-stone-800">{drivers.filter(d => d.isActive).length}</p>
                <p className="text-[10px] font-black uppercase text-stone-400">Active Drivers</p>
              </div>
              <div className="bg-white rounded-2xl border border-stone-100 p-4 text-center">
                <p className="text-3xl font-black text-stone-800">{deliveries.filter(d => d.status === DeliveryStatus.DELIVERED).length}</p>
                <p className="text-[10px] font-black uppercase text-stone-400">Completed</p>
              </div>
            </div>

            {/* Default driver at a glance */}
            <div className="bg-green-50 border border-green-100 rounded-2xl p-4 flex items-center gap-3">
              ...
              <div>
                <p className="text-[9px] font-black uppercase text-green-600">Default Driver</p>
                <p className="font-black text-green-800">{currentDefaultDriver?.name || 'Katie'}</p>
              </div>
            </div>

            <p className="text-center text-xs text-stone-400 pt-4">
              Tap a tile above to get started
            </p>
          </div>
        )}
